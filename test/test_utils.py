...
class TestUtils(unittest.TestCase):
    def test_fill(self):
        ...
        filled_path = utils.fill(path, 1, 0.2)
        ...
        filled_path = utils.fill(path, 1, 0.75)
        self.assertEqual(len(filled_path), len(path))
        np.testing.assert_equal(filled_path[0], path[0])
        np.testing.assert_equal(filled_path[-1], path[-1])

        filled_path = utils.fill(path, 4, 0.1)
        ...
        with self.assertRaisesRegex(ValueError, "intermediate_points must be > 0"):
            utils.fill(path, 0, 1)
            utils.fill(path, -5, 1)
