...
if __name__ == '__main__':
    visualize, use_obstacles, seed = panda_utils.parse_panda_args("Run RRT on a franka panda model and visualize the resulting paths.")
    model, path, shortcut_path, joint_qpos_addrs = panda_utils.rrt_panda(use_obstacles, seed)
    if visualize:
        ...
        with mujoco.viewer.launch_passive(model=model, data=data, show_left_ui=False, show_right_ui=False) as viewer:
            ...
            ex_utils.add_path(viewer.user_scn, model, panda_utils._PANDA_EE_SITE, joint_qpos_addrs, path, [1.0, 0.0, 0.0, 1.0])
            ex_utils.add_path(viewer.user_scn, model, panda_utils._PANDA_EE_SITE, joint_qpos_addrs, shortcut_path, [0.0, 1.0, 0.0, 1.0])
